...
def _build_model():
    ...
    gru_cell = tf.contrib.rnn.DropoutWrapper(gru_cell,
                                             output_keep_prob=float(config['MODEL INFO']['dropout_keep_prob']))
    ...
    loss = tf.reduce_sum(
        tf.multiply(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=sequence_output, labels=labels_reshaped),
                    zero_mask)) / num_nonzero
